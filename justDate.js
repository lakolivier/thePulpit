...
$(function() {
    $.get("justDate.php", function(jsonObj) {
        ...
        $("#allYears").append('<div class = "card bg-light rounded-0 col-md-4 my-3 border-0 mx-auto" id = "' + holdDate + '"></div>');
        ...
        for (let i = 1; i < $(jsArr).length; i++) {
            ...
            $("#" + lastDate).after('<div class = "card bg-light rounded-0 col-md-4 my-3 border-0 mx-auto" id = "' + holdDate + '"></div>');
            $("#" + holdDate).append('<h2 class = "card-title hovElement"><a href = "sermonDate.html#' + holdDate + '" class = "text-dark text-decoration-none">' + holdDate + "</a></h2>");
            lastDate = holdDate;
        }
    });
