$(function() {
    $.get("justTags.php", function(jsonObj) {
        ...
        let lastTag = trimTag;
        $("#allTags").append('<div class = "card bg-light rounded-0 col-sm-4 m-3 border-0 text-center" id = "' + trimTag + '"></div>');
        ...
        for (let i = 1; i < $(jsArr).length; i++) {
            ...
            $("#" + lastTag).after('<div class = "card bg-light rounded-0 col-sm-4 m-3 border-0 text-center" id = "' + trimTag + '"></div>');
